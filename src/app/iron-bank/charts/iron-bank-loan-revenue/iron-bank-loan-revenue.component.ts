...
@Component({
  selector: 'app-iron-bank-loan-revenue',
  templateUrl: './iron-bank-loan-revenue.component.html',
  styleUrls: ['./iron-bank-loan-revenue.component.css']
})
export class IronBankLoanRevenueComponent implements OnInit {
  // constants
  ...
  async handleData(data: QueryResult) {
    ...
    // @dev WETH reserve factor changes at t-15 and t-16 and chart doesn't factor that change in. need to fix.
    for (let t in result) {
      if (t !== 'markets') {
        for (let m in result[t]) {
          ...
          // if (entry.label === "WETH") {
          //   console.log(t + " ----- " + parseFloat(market.reserveFactor) / 1e18)
          // }
    ...
    // let me = this.assetRevenue.find((m) => m.label === "WETH");
    // console.log(me);
  }
